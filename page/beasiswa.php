...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if ($update) {
		$sql = "UPDATE beasiswa SET nama_beasiswa='$_POST[nama_beasiswa]' WHERE kd_beasiswa='$_GET[key]'";
	} else {
		$sql = "INSERT INTO beasiswa VALUES (NULL, '$_POST[nama_beasiswa]')";
		$validasi = true;
	}

	if ($validasi) {
		$q = $connection->query("SELECT kd_beasiswa FROM beasiswa WHERE nama_beasiswa LIKE '%$_POST[nama_beasiswa]%'");
		if ($q->num_rows) {
	        ...
	        <div class="panel-body">
	            <form action="<?=$_SERVER['REQUEST_URI']?>" method="POST">
	                <div class="form-group">
	                    ...
	                    <input type="text" name="nama_beasiswa" class="form-control" <?= (!$update) ?: 'value="'.$row["nama_beasiswa"].'"' ?>>
	                </div>
